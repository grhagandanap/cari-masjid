...
## Phase 2: Database Schema Design (Mosque Domain)
*Create schemas for the core domain in `src/db/schema.ts`.*

- [ ] **2.1 Create `mosques` Table:**
  - Fields: `id` (uuid, pk), `name` (text), `latitude` (real), `longitude` (real), `address` (text), `website` (text), `contact` (text), `createdAt` (timestamp), `createdById` (uuid, fk to user).
- [ ] **2.2 Create `facilities` Table (or JSONb field on mosques):**
...
- [ ] **2.3 Create `mosque_photos` Table:**
  - Fields: `id` (uuid, pk), `mosqueId` (uuid, fk), `url` (text), `uploadedById` (uuid, fk).
- [ ] **2.4 Generate & Push Migrations:**
...
- [ ] **3.1 Haversine Distance Utility (`src/utils/distance.ts`):**
  - Export a function `calculateDistance(lat1, lon1, lat2, lon2)` that returns the distance in kilometers.
- [ ] **3.2 Geolocation Hook (`src/hooks/use-geolocation.ts`):**
...
## Phase 5: UI - Discovery & Home Page
*Building the main user experience.*

- [ ] **5.1 Build Landing Page Hero (`src/routes/index.tsx`):**
...
- [ ] **5.2 Create `MosqueCard` Component (`src/components/MosqueCard.tsx`):**
  - Use shadcn `Card`.
  - Props: `name`, `distance`, `facilities` (boolean map), `thumbnailUrl`.
  - Render icons (e.g., Lucide React) for available facilities.
- [ ] **5.3 Implement Discovery Section (`src/routes/index.tsx`):**
...
## Phase 8: Nice to Have (Donations)
- [ ] **8.1 Add Donation Fields to Schema:**
  - Add `donationInfo` (text) or `bankAccountDetails` (text) to the `mosques` table.
- [ ] **8.2 Display Donation Info:**
